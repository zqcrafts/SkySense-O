...
class WindowAttention(nn.Module):
    ...
    def __init__(self,
        ...
        self.q = nn.Linear(dim + appearance_guidance_dim,
                           attn_dim,
                           bias=qkv_bias)
        self.k = nn.Linear(dim + appearance_guidance_dim,
                           attn_dim,
                           bias=qkv_bias)
    ...
    def forward(self, x, mask=None):
        ...
        v = self.v(x[:, :, :self.dim]).reshape(B_, N, self.num_heads,
                                               -1).permute(0, 2, 1, 3)
        ...
        if mask is not None:
            ...
            attn = attn.view(B_ // num_win, num_win, self.num_heads, N,
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, appearance_guidance):
        ...
        # print("self.input_resolution", self.input_resolution) # self.input_resolution [24, 24]
        # print("x.shape", x.shape) # x.shape torch.Size([131, 576, 128])  
        ...
        x_windows = x_windows.view(
            -1, self.window_size * self.window_size,
            x_windows.shape[-1])  # num_win*B, window_size*window_size, C
...
class SwinTransformerBlockWrapper(nn.Module):
    ...
    def __init__(self,
        ...
        self.padding_tokens = nn.Parameter(torch.zeros(
            1, 1, dim)) if pad_len > 0 else None
        self.padding_guidance = nn.Parameter(
            torch.zeros(1, 1, appearance_guidance_dim)
        ) if pad_len > 0 and appearance_guidance_dim > 0 else None
...
class ClassTransformerLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.pool = nn.AvgPool2d(
            pooling_size) if pooling_size is not None else nn.Identity()
        ...
        self.pad_len = pad_len
        self.padding_tokens = nn.Parameter(torch.zeros(
            1, 1, hidden_dim)) if pad_len > 0 else None
        self.padding_guidance = nn.Parameter(torch.zeros(
            1, 1, guidance_dim)) if pad_len > 0 and guidance_dim > 0 else None
        ...
        *_, H_pool, W_pool = x_pool.size()
        ...
        if self.padding_tokens is not None:
            ...
            if orig_len < self.pad_len:
                # pad to pad_len
                padding_tokens = repeat(self.padding_tokens,
                                        '1 1 C -> B C T H W',
                                        B=B,
                                        T=self.pad_len - orig_len,
                                        H=H_pool,
                                        W=W_pool)
        ...
        if guidance is not None:
            if self.padding_guidance is not None:
                if orig_len < self.pad_len:
                    padding_guidance = repeat(self.padding_guidance,
                                              '1 1 C -> B T C',
                                              B=B,
                                              T=self.pad_len - orig_len)
                    guidance = torch.cat([guidance, padding_guidance], dim=1)
            guidance = repeat(guidance,
                              'B T C -> (B H W) T C',
                              H=H_pool,
                              W=W_pool)
        ...
        x_pool = rearrange(x_pool,
                           '(B H W) T C -> (B T) C H W',
                           H=H_pool,
                           W=W_pool)
        x_pool = F.interpolate(x_pool,
                               size=(H, W),
                               mode='bilinear',
                               align_corners=True)
...
class AggregatorResNetLayer(nn.Module):
    ...
    def __init__(self, hidden_dim=64, appearance_guidance=512) -> None:
        super().__init__()
        self.conv_linear = nn.Conv2d(hidden_dim + appearance_guidance,
                                     hidden_dim,
                                     kernel_size=1,
                                     stride=1)
    ...
    def forward(self, x, appearance_guidance):
        ...
        x = rearrange(x, 'B C T H W -> (B T) C H W')
        appearance_guidance = repeat(appearance_guidance,
                                     'B C H W -> (B T) C H W',
                                     T=T)
...
class Aggregator(nn.Module):
    ...
    def __init__(
        ...
        """
        Cost Aggregation Model for SkySense-O
        Args:
        ...
        self.conv1 = nn.Conv2d(prompt_channel,
                               hidden_dim,
                               kernel_size=7,
                               stride=1,
                               padding=3)
        ...
        self.guidance_projection = nn.Sequential(
            nn.Conv2d(appearance_guidance_dim,
        ...
        self.head = nn.Conv2d(decoder_dims[1],
                              1,
                              kernel_size=3,
                              stride=1,
                              padding=1)
    ...
    def feature_map(self, img_feats, text_feats):
        ...
        img_feats = repeat(img_feats,
                           "B C H W -> B C T H W",
                           T=text_feats.shape[1])
        text_feats = F.normalize(text_feats, dim=-1)  # B T P C
        text_feats = text_feats.mean(
            dim=-2)  # average text features over different prompts
        text_feats = F.normalize(text_feats, dim=-1)  # B T C
        text_feats = repeat(text_feats,
                            "B T C -> B C T H W",
                            H=img_feats.shape[-2],
                            W=img_feats.shape[-1])
    ...
    def correlation(self, img_feats, text_feats):
        ...
        # print("image_features", img_feats)
        # print("text_features",  text_feats)
        # print("corr", corr)
        # exit()
        # corr_visual = corr.clone()
        # # print(corr_visual.shape)  # bpthw   # torch.Size([1, 1, 171, 24, 24])
        # corr_visual = corr_visual[:,0,:,:,:].permute(0,2,3,1)
        # corr_visual = corr_visual.cpu().detach().numpy() 
        ...
        # class_list = ["way", "tree", "meadow", "clearing", "structure", "tenement", "wasteland", "parking lot", "lane", "car", "concrete floor", "highroad", "plowland", "field", "water", "wind turbine", "lawn", "ridge", "fence", "tennis court", "baseball field", "riverbank", "bush", "green belt", "beach", "conduit", "bridge", "aerodrome", "ditch", "ocean", "graveyard", "the ship", "rock", "soccer field", "sewage treatment plant", "aeroplane", "storage tank", "stadium", "vegetation", "shore", "dune", "mountainous region", "impoundment", "isolation strip", "misc", "sand", "plot", "roundabout", "playground", "building", "rest area", "workshop", "site", "planting region", "train", "concrete road", "massif", "dam", "crop", "riffle", "seaboard", "train track", "block", "parking area", "bench", "forest land", "debris area", "terraced field", "river bed", "plastic greenhouse", "separator", "watercourse", "street lamp", "grandstand", "hamlet", "gravel land", "park", "rockily", "container", "airstrip", "dumping ground", "solar power plant", "stash", "gazebo", "reservoir", "equipment", "industrial park", "green plant", "rill", "pitcher", "canal", "cloud", "the helipad", "seaway", "railway station", "mountain road", "roof", "refinery", "forest", "quay", "road", "orchard", "solar panel", "golf course", "greenhouse", "path", "basketball court", "factory", "expressway", "rail", "tarmac", "unpaved road", "parking space", "runway", "brushwood", "island", "mountain range", "cistern", "unit", "substation", "boat", "courtyard", "desert", "grove", "living quarter"]
        # class_list = ["airplane", "airport", "baseball field", "basketball court", "bridge", "expressway service area", "dam", "golf field", "harbor", "ship", "football field", "storage tank", "tennis court", "train station", "vehicle", "windmill", "swimming pool", "impervious surfaces", "building", "low vegetation", "tree", "way", "clearing", "parking lot", "wasteland", "highroad", "lane", "field", "water", "river", "cropland", "plowland", "lake", "unpaved road", "pond", "expressway", "fence", "solar panel", "path", "unit", "ridge", "brushwood", "sandpit", "runway", "greenhouse", "footpath", "parking space", "courtyard", "mountain range", "island", "beach", "orchard", "roof", "site", "rock", "graveyard", "massif", "block", "planting region", "stadium", "sand", "lake water", "street lamp", "seawater", "train track", "house", "plot", "conduit", "impoundment", "sewage treatment plant", "shore", "dune", "ditch", "park", "river water", "seaboard", "forest land", "crop", "color steel house", "bench", "watercourse", "reservoir", "container", "isolation strip", "stash", "parking area", "train", "small courtyard", "afforestation", "farm", "refinery", "street", "gritty land", "crossroad", "trail", "auxiliary road", "spectator seat", "plastic greenhouse", "terraced field", "airstrip", "river bed", "seat", "bare land", "rill", "riffle", "nonmotorized lane", "telegraph pole", "misc", "construction site", "separator", "grandstand", "hamlet", "cultivated land", "gravel land", "gully", "oil tank", "mountain road", "rockily", "dumping ground", "solar power plant", "gazebo", "a small island", "equipment", "industrial park", "green plant", "sewage treatment tank", "pitcher", "canal", "cloud", "the helipad", "seaway"]
        # for class_index in range(corr_visual.shape[-1]):
        #     sub_corr_visual = corr_visual[0,:,:, class_index]
        #     # print(sub_corr_visual.shape)
        #     sub_corr_visual = cv2.normalize(sub_corr_visual, None, 0, 255, cv2.NORM_MINMAX)
        #     sub_corr_visual = sub_corr_visual.astype('uint8')
        #     cv2.imwrite(f"/gruntdata/rs_nas/workspace/xingsu.zq/CATSEG_skysense/debug/{class_list[class_index]}.png", sub_corr_visual)
        #     exit()
    ...
    def upsample(self, x):
        ...
        corr_embed = F.interpolate(corr_embed,
                                   scale_factor=2,
                                   mode='bilinear',
                                   align_corners=True)
        corr_embed = rearrange(corr_embed, '(B T) C H W -> B C T H W', B=B)
    ...
    def forward(self, img_feats, text_feats, appearance_guidance):
        ...
        # visual_corr
        # corr_visual = corr.clone()
        # print(corr_visual.shape)
        # corr_visual = corr_visual[:,0,:,:,:].permute(0,2,3,1)
        # corr_visual = corr_visual.cpu().detach().numpy()  # bpthw
        # cv2.imwrite("1.png", corr_visual)
        # exit()
        ...
        if self.pad_len > 0 and text_feats.size(1) > self.pad_len:
            ...
            th_text = torch.gather(th_text,
                                   dim=1,
                                   index=classes[..., None, None].expand(
                                       -1, -1, th_text.size(-2),
                                       th_text.size(-1)))
        ...
        if classes is not None:
            out = torch.full(
                (logit.size(0), orig_clases, logit.size(2), logit.size(3)),
                -100.,
                device=logit.device)
            out.scatter_(dim=1,
                         index=classes[..., None,
                                       None].expand(-1, -1, logit.size(-2),
                                                    logit.size(-1)),
                         src=logit)
            logit = out
            # (B T H W)
