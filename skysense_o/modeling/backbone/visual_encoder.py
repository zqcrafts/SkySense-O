...
from mmcls.models.utils import (PatchMerging, ShiftWindowMSA, WindowMSAV2,resize_pos_embed, to_2tuple)
...
class SwinBlockV2Sequence(BaseModule):
    ...
    def forward(self, x, in_shape, keep_res=False):
        if self.downsample:
            if keep_res:
                out_shape = in_shape
                x = x.repeat_interleave(4, -1)
                x = self.downsample.norm(self.downsample.reduction(x))
            else:
...
class SwinTransformerV2(BaseBackbone):
    ...
    def __init__(self,
        ...
        # print("self.patch_resolution", self.patch_resolution)
    ...
    def forward(self, x):
        ...
        for i, stage in enumerate(self.stages):
            if i >= 3:
                x, hw_shape = stage(x, hw_shape, keep_res=True)
            else:
