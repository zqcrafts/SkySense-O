...
class ResidualAttentionBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, prompt=None):
        # print("before attention", x)
        # self.attn.print_parameters()
        # print("x_shape", x.shape)
        # print("self.attn_mask", self.attn_mask)
        

        # del x
        # test_ln_feat = torch.load("/gruntdata/rs_nas/workspace/xingsu.zq/CAT-Seg/ln_feat.pt")
        # print("self.ln_1(x)", self.ln_1(x).shape)
        # print("test_ln_feat", test_ln_feat.shape)
        # print("after attention", x + self.attention(self.ln_1(x)))
        # print("after attention", self.attention(test_ln_feat))
        # exit()


        x = x + self.attention(self.ln_1(x))
        
        # print("after attention", self.attention(self.ln_1(x)))
    ...
    def forward_dense(self, x: torch.Tensor, prompt=None):
        ...
        if self.attn.in_proj_weight is not None:
            ...
            proj = torch.cat((self.attn.q_proj_weight, self.attn.k_proj_weight,
                              self.attn.v_proj_weight),
                             dim=0)
    ...
    def forward(self, x: torch.Tensor, dense=False, prompt=None):
        for i, resblock in enumerate(self.resblocks):
            if self.prompt_length > 0 and i < self.prompt_depth:
                x = torch.cat(
                    (x[0:1, :, :],
                     self.prompt_tokens[i].repeat(x.shape[1], 1,
                                                  1).permute(1, 0,
                                                             2), x[1:, :, :]))
...
class VisualTransformer(nn.Module):
    ...
    def print_parameters(self):
        for name, param in self.named_parameters():
            ...
            #     print(f"Parameter name: {name}, Parameter shape: {param.dtype}")
            # if name == "conv1.bias":
            #     print(f"Parameter name: {name}, Parameter shape: {param.dtype}")

    def forward(self, x: torch.Tensor, dense=False):
        # print("clip_images_resized", x)
        # self.print_parameters()
        # print("before_conv:", x.shape)
        x = self.conv1(x).to(torch.float32)  # shape = [*, width, grid, grid]
        # print("conv", x.shape)
        ...
        if dense and (x.shape[1] != self.positional_embedding.shape[0]):
            ...
            # print("1", self.resized_pos_embed(self.input_resolution,
                                        #    x.shape[1]).to(x.dtype))
        else:
            x = x + self.positional_embedding.to(x.dtype)
            # print("2", self.positional_embedding.to(x.dtype))
...
def build_model(state_dict: dict, prompt_depth=0, prompt_length=0):
    ...
    if vit:
        ...
        if "visual.transformer.resblocks.0.attn.q_proj_weight" in state_dict:
            print("find q_proj_weight")
            vision_layers = len([
                k for k in state_dict.keys()
                if k.startswith("visual.") and k.endswith(".attn.q_proj_weight")
            ])
        else:
            vision_layers = len([
                k for k in state_dict.keys()
        ...
        # print("vision_width", vision_width)  # 1024
        # print("vision_layers", vision_layers)
        # print("vision_patch_size", vision_patch_size)  #  14
        # print("image_resolution", image_resolution)  # 336  14*24
        # exit()
    ...
    # print(new_state_dict['visual.transformer.resblocks.0.attn.q_proj_weight'])
    # print(new_state_dict['visual.transformer.resblocks.0.attn.k_proj_weight'])
    # print(new_state_dict['visual.transformer.resblocks.0.attn.v_proj_weight'])
    # print(new_state_dict['visual.transformer.resblocks.0.attn.in_proj_bias'])
    # exit()
